<template>
  <div
    ...
    v-bind:class="{ 'boxes-rolling': game !== null && game.state === 'rolling' }"
  >
    ...
    <transition-group class="boxes-list" name="list" tag="div">
      <button
        ...
        v-bind:class="{
          'button-active':
            (index === 0 && ['created', 'countdown', 'pending'].includes(game.state) === true) ||
...
export default {
  ...
  data() {
    return {
      unboxImagePath: import.meta.env.VITE_VUE_APP_BACKEND_URL
    }
  },
  methods: {
...
.battles-game-boxes .selector-inner {
  ...
  align-items: center;
  background: radial-gradient(60% 60% at 50% 50%, rgba(10, 238, 179, 0.2) 0%, rgba(0, 0, 0, 0) 100%),
