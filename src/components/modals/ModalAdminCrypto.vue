    ...
    <div class="crypto-element element-currency">
      ...
      <div class="element-content">
        <div class="content-inner">
          {{ (modalsData.transaction.data.cryptoAmount / 100000000).toFixed(8) }}
        </div>
        <img
          v-bind:src="'@/assets/img/cashier/' + modalsData.transaction.data.currency + '.png'"
          alt="icon"
        />
        <button
          v-on:click="
            modalCopyButton((modalsData.transaction.data.cryptoAmount / 100000000).toFixed(8))
          "
          class="button-copy"
        >
          <svg
  ...
  methods: {
    ...
    modalApproveButton() {
      if (this.modalTransactionHash === null || this.modalTransactionHash.trim() === '') {
        this.notificationShow({
          type: 'error',
          message: 'Your entered transaction hash is invalid.'
        })
        return
      }
      ...
      const data = {
        ...
        transactionHash: this.modalTransactionHash
      }
    ...
    modalCancelButton() {
      ...
      this.modalsSetData({
        ...
        messageConfirm: 'Please confirm that you want cancel the crypto transaction.',
        data: { action: 'cancel', transactionId: this.modalsData.transaction._id }
      })
