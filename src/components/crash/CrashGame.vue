      ...
      <div v-else-if="crashGame.state === 'rolling'" class="inner-rolling">
        <div class="rolling-multiplier">
          <span class="gradient-green">{{
            parseFloat(Math.floor(crashMultiplier / 1000) / 100).toFixed(2)
          }}</span>
        </div>
        ...
        <transition name="fade">
          <div
            v-if="
              ...
              crashBets.some(
                (element) =>
                  element.user._id === authUser.user._id && element.multiplier === undefined
              ) === true
            "
...
export default {
  ...
  computed: {
    ...
    crashGetPayoutAmount() {
      return Math.floor(
        this.crashBets[
          this.crashBets.findIndex((element) => element.user._id === this.authUser.user._id)
        ].amount *
...
.crash-game {
  ...
  background: linear-gradient(0deg, rgba(5, 31, 51, 0.4), rgba(5, 31, 51, 0.4)),
    radial-gradient(100% 100% at 50% 50%, rgba(156, 88, 223, 0.3) 0%, rgba(0, 0, 0, 0) 100%),
