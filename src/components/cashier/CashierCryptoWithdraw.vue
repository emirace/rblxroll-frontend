<template>
  <div class="cashier-crypto-withdraw">
    <div class="withdraw-amount">
      ...
      <p>
        Enter the amount of Robux you would like to withdraw. The network fees will be deducted from
        your withdraw amount. Average network fees are
        <span>${{ modalFormatValue(cashierGetFee) }}</span
        >.
      </p>
    ...
    <button
      v-on:click="cashierWithdrawButton()"
...
export default {
  ...
  methods: {
    ...mapActions(['notificationShow', 'cashierSendCryptoWithdrawDepositSocket']),
    ...
    cashierCoinInput() {
      ...
      this.cashierCryptoAmount = parseFloat(
        (1 / (this.cashierCryptoData.prices[this.modalsData.currency].price / 1000)) *
    ...
    cashierWithdrawButton() {
      ...
      if (this.cashierAddress === null || this.cashierAddress.trim() === '') {
        this.notificationShow({
          type: 'error',
          message: 'You need to enter a valid ' + this.modalsData.currency + ' withdraw address.'
        })
        return
      }

      const data = {
        currency: this.modalsData.currency,
        amount: amount,
  ...
  computed: {
    ...mapGetters(['socketSendLoading', 'modalsData', 'cashierCryptoData']),
    cashierGetName() {
  ...
  watch: {
    'cashierCryptoData.prices': {
      handler() {
        ...
        this.cashierCryptoAmount = parseFloat(
          (1 / (this.cashierCryptoData.prices[this.modalsData.currency].price / 1000)) * 100
        ).toFixed(8)
