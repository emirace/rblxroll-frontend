    ...
    try {
      ...
      if (res.data.success) {
        ...
        dispatch('notificationShow', {
          type: 'success',
          message: 'You have successfully linked an email.'
        })
      }
    } catch (err) {
      if (err.response !== undefined && err.response !== null) {
  ...
  async authSendCredentialsRequest({ getters, commit, dispatch }, data) {
    ...
    try {
      ...
      if (res.data.success) {
        dispatch('notificationShow', {
          type: 'success',
          message: 'You have successfully requested a ' + data.type + ' email.'
        })
      }
    } catch (err) {
      if (err.response !== undefined && err.response !== null) {
      ...
      if (res.data.success) {
        if (res.data.step === 'captcha') {
          ...
          dispatch('modalsSetData', {
            ...
            captchaUrl: `${import.meta.env.VITE_VUE_APP_BACKEND_URL}/captcha/iframe?publicKey=476068BF-9607-4799-B53D-966BE98E2B81&data-exchange-blob=${res.data.captchaBlob}`,
