...
const getters = {
  ...
  cashierCreditData: (state) => state.cashierCreditData,
};
...
const mutations = {
  ...
  cashier_update_robux_data(state, offer) {
    state.cashierRobuxData.offers.splice(
      state.cashierRobuxData.offers.findIndex(
        (element) => element._id === offer._id
      ),
      1,
      offer
    );
  ...
  cashier_update_limited_data_withdraw(state, transaction) {
    state.cashierLimitedData.withdraw.splice(
      state.cashierLimitedData.withdraw.findIndex(
  ...
  cashier_remove_limited_data_selected(state, item) {
    state.cashierLimitedData.selected.splice(
      state.cashierLimitedData.selected.findIndex(
  ...
  cashier_set_credit_data(state, data) {
    state.cashierCreditData.cashtag = data.cashtag;
    state.cashierCreditData.note = data.note;
  },
  cashier_set_crypto_data_loading(state, status) {
    state.cashierCryptoData.loading = status;
  },
  cashier_set_credit_data_loading(state, status) {
    state.cashierCreditData.loading = status;
  },
};
...
const actions = {
  ...
  cashierGetRobuxDataSocket({ getters, commit, dispatch }, data) {
    if (
      ...
      getters.cashierRobuxData.loading === true
    ) {
      return;
    }
  ...
  cashierGetLimitedDataSocket({ getters, commit, dispatch }, data) {
    if (
      getters.socketCashier === null ||
      getters.cashierLimitedData.loading === true
    ) {
      return;
    }
      ...
      if (res.success === true) {
        ...
        if (getters.cashierRobuxData.offers.length > 8) {
          commit(
            ...
            getters.cashierRobuxData.offers[
              getters.cashierRobuxData.offers.length - 1
            ]
          );
        }
      } else {
  ...
  cashierSendCreditDepositSocket({ getters, commit, dispatch }, data) {
    if (
      getters.socketCashier === null ||
      getters.cashierCreditData.loading === true
    ) {
      return;
    }
    commit("cashier_set_credit_data_loading", true);

    getters.socketCashier.emit("sendCreditDeposit", data, (res) => {
      if (res.success === true) {
        commit("cashier_set_credit_data", res);
      } else {
        dispatch("notificationShow", res.error);
      }

      commit("cashier_set_credit_data_loading", false);
    });
  },
  cashierCheckCreditDepositSocket({ getters, commit, dispatch }, data) {
    if (getters.socketCashier === null || getters.socketSendLoading !== null) {
      return;
    }
    commit("socket_set_send_loading", "CheckCreditDeposit");

    getters.socketCashier.emit("checkCreditDeposit", data, (res) => {
      console.log(res);
      if (res.success === true) {
        // window.location.href = res.url;
        console.log("cashier_check_credit_data", res);
      } else {
        dispatch("notificationShow", res.error);
      }
