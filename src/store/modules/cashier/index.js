...
const getters = {
  ...
  cashierCashappData: (state) => state.cashierCashappData
}
...
const mutations = {
  ...
  cashier_update_robux_data(state, offer) {
    state.cashierRobuxData.offers.splice(
      state.cashierRobuxData.offers.findIndex((element) => element._id === offer._id),
      1,
      offer
    )
  },
  ...
  cashier_update_limited_data_withdraw(state, transaction) {
    state.cashierLimitedData.withdraw.splice(
      state.cashierLimitedData.withdraw.findIndex((element) => element._id === transaction._id),
      1,
      transaction
    )
  },
  ...
  cashier_remove_limited_data_selected(state, item) {
    state.cashierLimitedData.selected.splice(
      state.cashierLimitedData.selected.findIndex((element) => element._id === item._id),
      1
    )
  },
  ...
  cashier_set_cashapp_data(state, data) {
  ...
  cashier_set_cashapp_data_loading(state, status) {
    state.cashierCashappData.loading = status
  },
  cashier_set_cashapp_data_show(state, status) {
    state.cashierCashappData.show = status
  }
}
...
const actions = {
  ...
  cashierSocketLimitedTransaction({ getters, commit }, data) {
    if (getters.cashierLimitedData.withdraw !== null) {
      if (data.transaction.state === 'canceled') {
        commit('cashier_remove_limited_data_withdraw', data.transaction)
      } else if (
        getters.cashierLimitedData.withdraw.some(
          (element) => element._id === data.transaction._id
        ) === true
      ) {
        commit('cashier_update_limited_data_withdraw', data.transaction)
      } else {
        commit('cashier_add_limited_data_withdraw', data.transaction)
      }
    }
  },
  cashierSocketCryptoTransaction({ getters, commit }, data) {
    if (data.transaction.state === 'completed') {
      dispatch('notificationShow', {
        type: 'success',
        message:
          'Your ' + data.transaction.data.currency + ' deposit has been credited to your balance.'
      })
    }
  },
  cashierGetRobuxDataSocket({ getters, commit, dispatch }, data) {
    if (getters.socketCashier === null || getters.cashierRobuxData.loading === true) {
      return
    }
  ...
  cashierGetLimitedDataSocket({ getters, commit, dispatch }, data) {
    if (getters.socketCashier === null || getters.cashierLimitedData.loading === true) {
      return
    }
  ...
  cashierSendRobuxDepositSocket({ getters, commit, dispatch }, data) {
    ...
    getters.socketCashier.emit('sendRobuxDeposit', data, (res) => {
      if (res.success === true) {
        ...
        if (getters.cashierRobuxData.offers.length > 8) {
          commit(
            'cashier_remove_robux_data',
            getters.cashierRobuxData.offers[getters.cashierRobuxData.offers.length - 1]
          )
        }
      } else {
        dispatch('notificationShow', res.error)
  ...
  cashierSendLimitedDepositSocket({ getters, commit, dispatch }, data) {
    ...
    getters.socketCashier.emit('sendLimitedDeposit', data, (res) => {
      if (res.success === true) {
        ...
        dispatch('notificationShow', {
          type: 'success',
          message: 'You have successfully created your limited listing.'
        })
      } else {
        dispatch('notificationShow', res.error)
      }
  ...
  cashierSendLimitedWithdrawSocket({ getters, commit, dispatch }, data) {
    ...
    getters.socketCashier.emit('sendLimitedWithdraw', data, (res) => {
      if (res.success === true) {
        if (res.transaction.state === 'completed') {
          dispatch('notificationShow', {
            type: 'success',
            message: 'You have successfully withdrawed the limited listing.'
          })
        }
      } else {
        dispatch('notificationShow', res.error)
    ...
    getters.socketCashier.emit('sendLimitedCancel', data, (res) => {
      if (res.success === true) {
        if (getters.modalsShow === 'LimitedsItem') {
          dispatch('modalsSetShow', null)
          setTimeout(() => {
            dispatch('modalsSetShow', 'Limiteds')
          }, 200)
        }

        dispatch('notificationShow', {
          type: 'success',
          message: 'You have successfully canceled your limited listing.'
        })
      } else {
        dispatch('notificationShow', res.error)
      }
  ...
  cashierSendCashappDepositSocket({ getters, commit, dispatch }, data) {
    if (getters.socketCashier === null || getters.cashierCashappData.loading === true) {
      return
    }

    if (!data.amount || isNaN(data.amount) || data.amount < 1.42) {
      dispatch('notificationShow', {
        type: 'error',
    ...
    commit('cashier_set_cashapp_data_loading', true)
    commit('cashier_set_cashapp_data_show', true)
    getters.socketCashier.emit('sendCashappDeposit', data, (res) => {
      if (res.success === true) {
        commit('cashier_set_cashapp_data', res)
        console.log(res)
      } else {
        dispatch('notificationShow', res.error)
        commit('cashier_set_cashapp_data_show', false)
      }

      commit('cashier_set_cashapp_data_loading', false)
    })
  },
  cashierCheckCashappDepositSocket({ getters, commit, dispatch }, data) {
    if (getters.socketCashier === null || getters.socketSendLoading !== null) {
      return
    }
    commit('socket_set_send_loading', 'CheckCashappDeposit')

    getters.socketCashier.emit('checkCashappDeposit', data, (res) => {
      console.log(res)
      if (res.success === true) {
        // window.location.href = res.url;
        console.log('cashier_check_cashapp_data', res)
      } else {
        dispatch('notificationShow', res.error)
      }
  ...
  cashierSendGiftRedeemSocket({ getters, commit, dispatch }, data) {
    ...
    getters.socketCashier.emit('sendGiftRedeem', data, (res) => {
      if (res.success === true) {
        dispatch('notificationShow', {
          type: 'success',
          message: 'You have successfully redeemed a gift code.'
        })
      } else {
        dispatch('notificationShow', res.error)
      }
